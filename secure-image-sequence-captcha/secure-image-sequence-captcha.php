...
define( 'SISC_VERSION', '1.4.0' ); // Versión incrementada
...
if ( ! class_exists( 'Secure_Image_Sequence_Captcha' ) ) {
    ...
    class Secure_Image_Sequence_Captcha {
        ...
        public function display_admin_notices() {
            ...
            if ( 'edit-' . SISC_TAXONOMY_SLUG === $screen->id ) {
                ...
                <div class="notice notice-info">
                    <p>
                        <strong><?php esc_html_e( 'Quick Tip:', 'secure-image-sequence-captcha' ); ?></strong>
                        <?php echo wp_kses_post( sprintf(
                            __( 'For a category to be used by the CAPTCHA, it must contain at least %d images.', 'secure-image-sequence-captcha' ),
                            SISC_TOTAL_IMAGES
                        ) ); ?>
                    </p>
                </div>
            ...
            if ( 'settings_page_' . SISC_SETTINGS_SLUG === $screen->id && 'custom' === $this->options['image_source'] ) {
                $status = $this->get_custom_category_status();

                if ( $status['total'] > 0 && empty($status['valid']) ) {
                    // Error: Hay categorías, pero ninguna es válida.
                    $taxonomy_url = admin_url( 'edit-tags.php?taxonomy=' . SISC_TAXONOMY_SLUG . '&post_type=attachment' );
                    ?>
                    <div class="notice notice-error is-dismissible">
                        <p>
                            <strong><?php esc_html_e( 'Action Required:', 'secure-image-sequence-captcha' ); ?></strong>
                            <?php echo wp_kses_post( sprintf(
                                __( 'The Image Sequence CAPTCHA is currently inactive because none of your CAPTCHA Categories have the required minimum of %1$d images. Please <a href="%2$s">update your categories</a> to activate protection.', 'secure-image-sequence-captcha' ),
                                SISC_TOTAL_IMAGES,
                    ...
                    $invalid_names = wp_list_pluck( $status['invalid'], 'name' );
                    ?>
                    <div class="notice notice-warning is-dismissible">
                        <p>
                            <strong><?php esc_html_e( 'Warning:', 'secure-image-sequence-captcha' ); ?></strong>
                            <?php esc_html_e( 'The following CAPTCHA categories have fewer than the required minimum of images and will be ignored:', 'secure-image-sequence-captcha' ); ?>
                            <ul style="list-style: disc; margin-left: 20px; margin-top: 5px; margin-bottom: 5px;">
                                <?php foreach ( $invalid_names as $name ) : ?>
                                    <li><?php echo esc_html( $name ); ?></li>
                                <?php endforeach; ?>
                            </ul>
                             <?php esc_html_e( 'The CAPTCHA will continue to work using only the valid categories.', 'secure-image-sequence-captcha' ); ?>
                        </p>
                    </div>
                    <?php
                }
        ...
        private function get_custom_category_status() {
            $all_terms = get_terms([ 'taxonomy' => SISC_TAXONOMY_SLUG, 'hide_empty' => false ]);
            if ( empty( $all_terms ) || is_wp_error( $all_terms ) ) { return ['total' => 0, 'valid' => [], 'invalid' => []]; }
            ...
            foreach ( $all_terms as $term ) {
                $query = new WP_Query([
                    ...
                    'tax_query' => [['taxonomy' => SISC_TAXONOMY_SLUG, 'field' => 'term_id', 'terms' => $term->term_id]],
                    'fields' => 'ids', 'no_found_rows' => false,
                ]);
                if ( $query->found_posts >= SISC_TOTAL_IMAGES ) { $valid_categories[] = $term; } else { $invalid_categories[] = $term; }
            }
            return [ 'total' => count($all_terms), 'valid' => $valid_categories, 'invalid' => $invalid_categories ];
        }
        ...
        public function display_captcha_in_comments() { $challenge_data = $this->generate_captcha_challenge(); if ( !$challenge_data ) { echo '<p class="sisc-error"><em>' . esc_html__( 'CAPTCHA could not be generated due to a configuration issue. Please contact the site administrator.', 'secure-image-sequence-captcha' ) . '</em></p>'; return; } $this->render_captcha_html( $challenge_data, 'comments' ); }
        public function display_captcha_in_login() { $challenge_data = $this->generate_captcha_challenge(); if ( !$challenge_data ) { echo '<p class="sisc-error login-error"><em>' . esc_html__( 'CAPTCHA generation failed. Please contact the site administrator.', 'secure-image-sequence-captcha' ) . '</em></p>'; return; } echo '<div style="margin-bottom: 15px;">'; $this->render_captcha_html( $challenge_data, 'login' ); echo '</div>'; }
        ...
        public function validate_login_captcha( $user, $username, $password ) { if ( empty( $this->options['enable_login'] ) ) { return $user; } if ( ! isset( $_POST['sisc_transient_key'] ) ) { error_log('[SISC] CAPTCHA validation skipped on login: sisc_transient_key not found. Check plugin configuration.'); return $user; } if ( is_wp_error( $user ) ) { return $user; } $validation_result = $this->perform_captcha_validation(); if ( is_wp_error( $validation_result ) ) { $wp_error = new WP_Error(); $wp_error->add( $validation_result->get_error_code(), $validation_result->get_error_message() ); return $wp_error; } return $user; }
        public function validate_register_captcha( $errors, $sanitized_user_login, $user_email ) { if ( empty( $this->options['enable_register'] ) ) { return $errors; } if ( ! isset( $_POST['sisc_transient_key'] ) ) { error_log('[SISC] CAPTCHA validation skipped on register: sisc_transient_key not found. Check plugin configuration.'); return $errors; } $validation_result = $this->perform_captcha_validation(); if ( is_wp_error( $validation_result ) ) { $errors->add( $validation_result->get_error_code(), $validation_result->get_error_message() ); } return $errors; }
        ...
        public function render_custom_taxonomy_column( $content, $column_name, $term_id ) {
            if ( 'sisc_count' === $column_name ) {
                $query_args = array( 'post_type' => 'attachment', 'post_status' => 'inherit', 'posts_per_page' => -1, 'tax_query' => array( array( 'taxonomy' => SISC_TAXONOMY_SLUG, 'field' => 'term_id', 'terms' => $term_id, ), ), 'fields' => 'ids', 'no_found_rows' => true, );
                $attachment_query = new WP_Query( $query_args );
                $count = $attachment_query->post_count;
                $term = get_term( $term_id );
                if ( is_wp_error( $term ) || ! $term ) { return $count; }
                $media_link = esc_url( admin_url('upload.php?taxonomy=' . SISC_TAXONOMY_SLUG . '&term=' . $term->slug ) );
